...
public class IssueManager {
	...
	public void assingTicket(int ticketId, String username) throws TicketException {
		...
		User u = elencoUtenti.get(username);
		t.setUtente(u);
		u.setBiglietto(t);
	...
	public void closeTicket(int ticketId, String description) throws TicketException {
		...
		User u = elencoUtenti.get(t.getUtente().getUsername());
		u.setBigliettoChiuso(t);
	}
	...
	public SortedMap<Ticket.Severity, Long> countBySeverityOfState(Ticket.State state) {
		...
		if (state != null) {
			...
			return biglietti.values().stream().filter((Ticket t) -> t.getState().equals(state))
					.collect(groupingBy(Ticket::getSeverity, () -> new TreeMap<Ticket.Severity, Long>(), counting()));

		}
		return biglietti.values().stream()
				.collect(groupingBy(Ticket::getSeverity, () -> new TreeMap<Ticket.Severity, Long>(), counting()));
	}
	...
	public List<String> topMaintainers() {
		...
		List<String> top = elencoUtenti.values().stream()
				.sorted((comparing((User u) -> u.getBigliettiChiusi().size(), reverseOrder()))
						.thenComparing((User u) -> u.getUsername()))
				.map(u -> u.getUsername() + ":" + u.getBigliettiChiusi().size()).collect(toList());
