...
public class Book {
	...
	public boolean checkTopics() {
		...
		List<Topic> argomentiCapitoliTeoria = new ArrayList<>();
		listaTeoria.stream()
		.map(e -> e.getTopics())
		.forEach(e -> argomentiCapitoliTeoria.addAll(e));
	
		return (argomentiCapitoliTeoria.containsAll(argomentiCapitoliEsercizi));
	}
